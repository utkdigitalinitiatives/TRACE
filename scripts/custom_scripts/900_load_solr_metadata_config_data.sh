...
if [ -f "$SHARED_DIR/configs/solr-tables/islandora_solr_fields.sql" ]; then
  ...
  drush -v sql-cli < "$SHARED_DIR"/configs/solr-tables/islandora_solr_fields.sql
  drush cc all
fi
...
if [ -f "$SHARED_DIR/configs/form-tables/xml_forms.sql" ]; then
    ...
    drush -v sql-cli < "$SHARED_DIR"/configs/form-tables/xml_forms.sql
    drush cc all
fi
...
if [ -f "$SHARED_DIR/configs/form-tables/xml_form_builder_form_associations.sql" ]; then
    ...
    drush -v sql-cli < "$SHARED_DIR"/configs/form-tables/xml_form_builder_form_associations.sql
    drush cc all
fi
