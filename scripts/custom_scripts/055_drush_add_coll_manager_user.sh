...
drush -r "$DRUPAL_HOME" user-information thesis_manager 2>&1 | grep '\[error\]' && drush -r "$DRUPAL_HOME" user-create thesis_manager --mail="vagrant+thesis_manager@localhost.com" --password="thesis_manager" && echo "Created thesis_manager user" || echo "The thesis_manager user already exists"
